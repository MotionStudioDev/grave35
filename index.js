...
/////////////////////// talep sistemi
const { Client, GatewayIntentBits, Collection, EmbedBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, ChannelType, SlashCommandBuilder } = require("discord.js");
const client = new Client({
  intents: [
    GatewayIntentBits.Guilds,
    GatewayIntentBits.GuildMessages,
  ...
  if (interaction.isButton()) {
    ...
    if (interaction.customId === "talep_ac") {
      try {
        ...
        console.error("Talep kanalı oluşturulamadı:", err);
        await interaction.reply({
          embeds: [
            new EmbedBuilder()
              .setColor("Red")
              .setTitle("❌ Talep Kanalı Oluşturulamadı")
              .setDescription("Bir hata oluştu. Lütfen bot yetkilerini kontrol et.")
              .setTimestamp()
          ],
          ephemeral: true
        });
